<template>
	<div>
		<!--begin::Modal-->
		<div class="modal fade" id="m_modal_create_geo" role="dialog" aria-hidden="true">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">
							{{ editing ? trans.get('universal.edit') : trans.get('universal.create') }}
						</h5>
						<button type="button" class="close" data-dismiss="modal">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body">
						<form class="m-form m-form--fit m-form--label-align-right" enctype="multipart/form-data">
							<div class="m-form__section m-form__section--first">
								<div class="form-group m-form__group row">
									<label class="col-lg-2 col-form-label">
										{{ trans.get('universal.type') }}
									</label>
									<div class="col-lg-8">
										<select class="form-control custom-select" id="type" 
										 v-model="geo.type"
										>
											<option v-for="type in types" 
											:value="type"
											>
												{{ trans.get('universal.' + type ) }}
											</option>
										</select>

										<span v-if="errors.type"class="m-form__help">{{ errors.type[0] }}</span>
									</div>
								</div>

								<div v-show="geo.type == 'quarter'" class="form-group m-form__group row">
									<label class="col-lg-2 col-form-label">
										{{ trans.get('universal.city') }}
									</label>
									<div class="col-lg-8">
										<select class="form-control custom-select" id="city_id" 
										 v-model="geo.city_id"
										>
											<option v-for="city in cities" 
											:value="city.id"
											>
												{{ city.name }}
											</option>
										</select>

										<span v-if="errors.city_id"class="m-form__help">{{ errors.city_id[0] }}</span>
									</div>
								</div>

								<div class="form-group m-form__group row">
									<label for="name" class="col-lg-2 col-form-label">{{ trans.get('universal.name') }}</label>
									<div class="col-lg-8">
										<input v-model="geo.name" type="text" class="form-control m-input" placeholder="" id="name">
										<span v-if="errors.name"class="m-form__help">{{ errors.name[0] }}</span>
									</div>
								</div>
							</div>
						</form>
					</div>

					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
						<button v-if="editing" type="button"
							v-bind:class="{'m-btn m-loader m-loader--light m-loader--right': saving}"
							class="btn btn-primary"
						 	@click="editGeo"
						 	:disabled="saving"
						 	>
							{{ trans.get('universal.edit') }}
						</button>

						<button v-else type="button" class="btn btn-primary"
							v-bind:class="{'m-btn m-loader m-loader--light m-loader--right': saving}" 
							@click="createGeo"
							:disabled="saving"
							>
							{{ trans.get('universal.create') }}
						</button>
					</div>
				</div>
			</div>
		</div>

		<!--end::Modal-->
	</div>
</template>

<script>
	import {onFileChange} from '../../../Helpers'
	import {setFileValue} from '../../../Helpers'

	class Geo{
		constructor(geo){
			this.id = geo.id || '';
			this.name = geo.name || '';
			this.type = geo.type || 'city';
			this.city_id = geo.city_id || '';
		}
	}

	export default{
		props: ['cities', 'types'],
		data(){
			return {
				geo: {},
				errors: {},
				editing: false,
				saving: false,
			}
		},
		mounted(){
			this.$parent.$on('create_geo', () => {
				this.editing = false;
				this.geo = new Geo({});
				this.errors = {};
			});

			this.$parent.$on('edit_geo', ({geo}) => {
				this.editing = true;
				this.geo = new Geo(geo);
				this.errors = {};

				this.$helpers.changeSelect('#type', this.geo.type);
			});
		},
		methods: {
			createGeo(){
				this.saving = true;
				axios.post('/api/geos', this.geo)
				.then(response => {
					this.saving = false;
					this.$parent.$emit('geo_created', response.data.data);

					this.$helpers.modalHide('#m_modal_create_geo');
					
				}).catch(error => {
					this.saving = false;
					this.errors = error.response.data.errors;
				});
			},
			editGeo(){
				this.saving = true;
				axios.put('/api/geos/' + this.geo.id, this.geo)
				.then(response => {
					this.saving = false;
					this.$parent.$emit('geo_updated', response.data.data);

					this.$helpers.modalHide('#m_modal_create_geo');
					
				}).catch(error => {
					this.saving = false;
					this.errors = error.response.data.errors;
				});
			},
		}
	}
</script>